...
const testInputP1 = await Deno.readTextFile("05/test-p1.txt");
const testInputP2 = await Deno.readTextFile("05/test-p1.txt");
const input = await Deno.readTextFile("05/input.txt");
Deno.test("p1 test", () => {
  assertEquals(problem1(testInputP1), 143);
});

Deno.test("p1 Solution", () => {
  assertEquals(problem1(input), 5747);
});
